...
> **中文摘要**：检测和修正技术文档中的"叙述重心错位"问题——当响亮但表层的实现细节抢了真正决定行为的架构性机制的风头时，读者会把心智模型锚定在错误的位置。通过"替换实验"判定每个细节的角色（架构性 / 传输性 / 配置性），在前处理阶段标注收集物，在后处理阶段检测并精准修正。后处理包含权威性校验步骤，确保重心迁移不引入技术语义错误。兼容 OpenClaw / CodeBuddy 原生 Skill 格式，也适用于 Claude Code、Cursor 等 AI coding agent。完整文档见下方。

An AgentSkill for detecting and fixing **Narrative Weight Misalignment** in technical documentation. Compatible with OpenClaw, CodeBuddy, and any AI coding agent.

## The Problem

Technical docs often give disproportionate emphasis to implementation details that are familiar or have catchy names (e.g., "event delegation", "virtual DOM"), while the mechanisms that actually determine user-observable behavior get buried as footnotes. This causes readers to build incorrect mental models anchored to the wrong concepts.

**Example**: A React event system article titled one of its "Three Core Mechanisms" as "Event Delegation" with a full chapter — but event delegation is just the signal delivery pipe. The actual core is Fiber-tree traversal, which was compressed into a few lines in a flow chart.
...
See [`examples/react-event-system/`](examples/react-event-system/) for a real before/after case study.

## File Structure

```
narrative-focus/
├── SKILL.md                          # Flow control: shared concepts + mode routing
├── references/
│   ├── pre-processing.md             # SOP for collection & labeling phase
│   └── post-processing.md            # SOP for detection, correction & verification
└── examples/
    └── react-event-system/
        ├── before.md                 # Original article with misalignment
        └── after.md                  # Corrected article
```
